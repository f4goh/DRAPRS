...
add oled display SSD1306 I2C 128x64
...
#ifdef AFFLCD
LiquidCrystal_SSD1306 lcd(SSD1306_SWITCHCAPVCC, SSD1306_I2C_ADDRESS);
#endif
...
void setup() {
  ...
  #ifdef AFFLCD
    lcd.begin(21, 7);  //7 lines *21 columns lcd char
  ...
  else
  {
    ...
    lcd.print(F(" minutes and "));
    lcd.print(neoDelay);
    lcd.println(F(" seconds"));
  }
    #endif
...
if (gps.available()) {
  ...
  previousMillisSerial = millis();
  if (Beacon.GPGGA.sync ==0) if (Beacon.GPGGA.fix == 1)  lcdState = 2; else lcdState = 3;

}
else
{
  if (EcratMillisSerial > 2000) {
    previousMillisSerial = millis();
    lcdState = 1;
  }
}
...
#ifdef AFFLCD
if (lcdOn == 1) {
  if (lcdStatePrec != lcdState)
  {
    switch (lcdState)
    {
      case 1 : lcd.setCursor(0, 6);                     //Print GPS info to lcd
        lcd.print(F("NO GPS Plugged "));
        break;
      case 2 : lcd.setCursor(0, 6);                     //Print GPS info to lcd
        lcd.print(F("GPS OK -Fix    "));
        break;
      case 3 : lcd.setCursor(0, 6);                     //Print GPS info to lcd
        lcd.print(F("GPS OK -No Fix "));
        break;
    }

    lcdStatePrec = lcdState;
  }
}
#endif
...
void EepromMenu()
{
  ...
  if (EEPROM.read(offsetEEPROM)!=0xff) updateAll();
  String cmd;
  String arg;
  byte argNull;
  car=Serial.read();
...
void saveEprom(char car, String arg)
{
  ...
  for (n=0;n<arg.length();n++)  EEPROM.write(offsetEEPROM+prtcar+n, arg.charAt(n));
  for (n=arg.length();n<nbcar;n++)  EEPROM.write(offsetEEPROM+prtcar+n, 0);
}
...
int updateValue(char car)
{
  ...
  int nbcar =int(pgm_read_word(&nbCharPtr[car-0x41]));
  char buffer[nbcar];
  for (byte n=0;n<nbcar;n++) buffer[n]=EEPROM.read(offsetEEPROM+prtcar+n);
  return atoi(buffer);
}
...
char updateValuechar(char car)
{
  ...
  return EEPROM.read(offsetEEPROM+prtcar);
}
...
void updateValuearray(char car, char *buf)
{
  ...
  for (byte n=0;n<nbcar;n++) buf[n]=EEPROM.read(offsetEEPROM+prtcar+n);
  buf[nbcar]=0;
  //Serial.print("----");
  //Serial.println(buf);
}
...
void updatetrack(char car, int ptrTrack, byte shiftEnable)
{
  ...
  for (m=0;m<nbcar;m++){ 
        c=EEPROM.read(offsetEEPROM+prtcar+m);
        if(c==0) c=c+0x20;
...
void sauveEeprom()
{
  for (int n=0;n<BUFFER_EEPROM;n++) {
    if (EEPROM.read(offsetEEPROM+n)!=buffer[n]) EEPROM.write(offsetEEPROM+n, (byte) buffer[n]);
  }
}
...
{
  for (int n=0;n<BUFFER_EEPROM;n++) {
    Serial.write(EEPROM.read(offsetEEPROM+n));
  }
}
...
void generateCsvFromEeprom()
{
  ...
  for (n=0;n<NB_PARAMS;n++) {
    ...
    for (m=0;m<nbcar;m++) {
      ...
      car=EEPROM.read(offsetEEPROM+prtcar+m);
